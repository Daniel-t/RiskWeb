...
function probSummary(data: TreeNodeData): string {
  const p = data.probability ?? data.fairInputs?.vulnerability;
  if (!p) return 'P=1';
  if (p.type === 'constant') return `P=${p.params.value}`;
  if (p.type === 'pert') return `P~${p.params.mode}`;
  if (p.type === 'lognormal') return `P LN`;
  return '';
}
