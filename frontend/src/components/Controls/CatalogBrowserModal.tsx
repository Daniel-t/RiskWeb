...
import { getAttackTechniques, getD3fendTechnique, getMappings } from '../../services/catalog';
...
const tactics = ['initial-access', 'execution', 'impact'] as const;
...
export function CatalogBrowserModal({
  ...
  const handleCreateControl = (defense: {
    id: string;
    name: string;
    category: string;
    ...
    onCreateFromCatalog({
      ...
      lefReduction: defense.suggestedLefReduction ?? {
        type: 'pert',
              ...
              {techniques.map((tech) => (
                <div
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    background:
                      selectedTech?.id === tech.id ? 'var(--bg-drop-highlight)' : 'transparent',
                    borderLeft:
                          ...
                          <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                            ...
                            {def.suggestedLefReduction?.type === 'pert' && (
                              <>
                                ...
                                | ~
                                {Math.round(
                                  (def.suggestedLefReduction.params as { mode: number }).mode * 100,
                                )}
                                % LEF reduction
                              </>
                            )}
